...
async function sendEmailNotification(payload) {
  try {
    ...
    const res = await fetch(EMAIL_FUNCTION_URL, {
      ...
      body: JSON.strinimport { useEffect, useMemo, useState } from 'react'
import { createClient } from '@supabase/supabase-js'
...
export default function App() {
  ...
  async function recordManualLeave() {
    ...
    await sendEmailNotification({
      ...
      reason: finalReason,
      days,
      deductDays: shouldDeductLeaveDays(finalReason),
    })
  ...
  async function submitLeaveRequest() {
    ...
    await sendEmailNotification({
      type: 'new_request',
      employeeName: myEmployee?.full_name || fullName,
      employeeEmail: myEmployee?.email || session.user.email,
      startDate,
      endDate,
      reason,
    })
  ...
  async function updateLeaveStatus(leave, status) {
    ...
    await sendEmailNotification({
      type: 'request_status',
      employeeName: emp?.full_name || 'Вработен',
      employeeEmail: emp?.email,
      startDate: leave.start_date,
      endDate: leave.end_date,
      reason: leaveReason,
      status,
      deductDays: status === 'approved' && shouldDeductLeaveDays(leaveReason),
    })
